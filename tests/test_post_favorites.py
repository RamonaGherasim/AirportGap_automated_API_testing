...
class TestPostFavorites:
    ...
    def test_post_favorites_authenticated_valid_id_no_note(self):
        # Create a favorite for airport id "LAE"
        airport_id = "LAE"
        ...
        # Delete list of favorites to avoid the status code 500 error which is returned when you
        # try to add the same airport to favorites more than once. This ensure the test can
        # be used in isolation to the other tests.
        delete_all_favorites(token)
